...
function DailyTaskCard({ task, onClick }: { task: Task; onClick: () => void }) {
  ...
  return (
    <div style={{
      ...
      {/* Timer — строго по bonuses.svg: rect 215×33 rx=16.5 fill #EEECF9, текст #02020E, формат XX д : XX ч : XX м : XX с */}
      {countdown && (
        <div style={{
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: '#02020E', letterSpacing: 0.3 }}>
            {countdown}
          </span>
        </div>
      )}
...
Если интересно, вот ссылка: ${url}`
  ...
  return (
    <div className="page" style={{ background: '#F5F5F5' }}>
      ...
      {/* Шапка по components.svg */}
      <AppHeader title="TASKCASH" />
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E5E6EE', flexShrink: 0 }}>
        {/* User row — avatar + name + chevron + level badge (header/user) */}
        <div onClick={() => navigate('/profile')} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 16px 10px', cursor: 'pointer' }}>
          {user?.photo_url ? (
      ...
      {/* Tab bar — pill style, фон серый как в bonuses.svg (#F5F5F5) */}
      <div style={{
        background: '#F5F5F5',
      ...
      <div className="scroll-area" style={{ background: '#F5F5F5' }}>

        {/* ══ TAB: Ежедневные ═══════════════════════════════════════════════ */}
        {tab === 'daily' && (
          <div style={{ padding: '12px 16px 24px', background: '#F5F5F5' }}>
        ...
        {tab === 'referrals' && (
          <>
            {/* Header: speaker (из bonuses.svg) + title; фон серый */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 16px 0', background: '#F5F5F5' }}>
              <SpeakerIllustration />
            ...
            {/* Stats card — только этот блок белый (0 рефералов / 0 заработано) */}
            <div style={{
              margin: '12px 16px 0',
            ...
            {bonuses && (bonuses.referral_reward != null || bonuses.referral_min_tasks != null) && (
              <div style={{
                margin: '12px 16px 0',
                background: '#E2F3EE',
                borderRadius: 12,
                ...
                <p style={{ fontSize: 12, fontWeight: 600, color: '#02020E', marginBottom: 4 }}>Расчёт начисления</p>
                <p style={{ fontSize: 12, color: '#4D536D', lineHeight: 1.4 }}>
                  {Number(bonuses.referral_reward ?? 0).toFixed(0)} ₽ за каждого реферала, выполнившего не менее {bonuses.referral_min_tasks ?? 0} заданий.
                </p>
              </div>
            )}
            ...
            {/* "Данные обновляются раз в 10 минут" — зелёный как в SVG */}
            <p style={{ fontSize: 11, color: ACCENT, textAlign: 'center', padding: '6px 16px 0', background: '#F5F5F5' }}>
              Данные обновляются раз в 10 минут
            </p>

            {/* Empty state — Label_2 по bonuses.svg: цвет #4D536D, крупнее и жирнее */}
            {refsCount === 0 && (
              <div style={{ textAlign: 'center', padding: '8px 16px 0', background: '#F5F5F5' }}>
                <p style={{ fontSize: 15, fontWeight: 700, color: '#4D536D', marginBottom: 6 }}>У вас пока нет рефералов</p>
                <p style={{ fontSize: 13, color: ACCENT, fontWeight: 600 }}>Поделитесь ссылкой с друзьями!</p>
              </div>
            )}

            {/* Referral link section */}
            <p style={{ fontSize: 13, fontWeight: 600, color: ACCENT, padding: '14px 16px 0', background: '#F5F5F5' }}>
              Ваша реферальная ссылка
            </p>

            {/* Link field + copy button */}
            <div style={{ display: 'flex', gap: 8, padding: '8px 16px 0', background: '#F5F5F5' }}>
              <div style={{
            ...
            {bonuses && bonuses.referrals.length > 0 && (
              <div style={{ padding: '16px 16px 0', background: '#F5F5F5' }}>
                {/* "👥 ДРУЗЬЯ" header */}
                    ...
                    <div key={ref.telegram_id} style={{
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                          {ref.status === 'blocked' ? (
                            <span style={{ fontSize: 10, fontWeight: 700, color: '#EF4444', background: '#FEE2E2', borderRadius: 20, padding: '2px 8px' }}>
                              🚫 Заблокирован
                            </span>
                          ) : ref.status === 'active' ? (
                            <span style={{ fontSize: 10, fontWeight: 700, color: '#047935', background: '#DCFCE7', borderRadius: 20, padding: '2px 8px' }}>
                              ✅ Активен
                            </span>
                          ) : (ref.tasks_completed ?? 0) > 0 ? (
                            <span style={{ fontSize: 10, fontWeight: 700, color: '#7C3AED', background: '#F3E8FF', borderRadius: 20, padding: '2px 8px' }}>
                              🔄 {ref.tasks_completed} {(ref.tasks_completed ?? 0) === 1 ? 'задание' : (ref.tasks_completed ?? 0) < 5 ? 'задания' : 'заданий'} выполнено
                            </span>
                          ) : (
                            <span style={{ fontSize: 10, fontWeight: 700, color: '#9B9FB0', background: '#F1F5F9', borderRadius: 20, padding: '2px 8px' }}>
                              🆕 Новый
                            </span>
                          )}
                        </div>
