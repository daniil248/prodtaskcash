...
import type { Transaction, TransactionType, Bonuses, Withdrawal, Profile } from '../types'
...
function formatPhoneInput(val: string): string {
  ...
  if (digits.length && (digits[0] === '8' || digits[0] === '7')) digits = '7' + digits.slice(1)
...
function GearIcon() {
  return (
    ...
    <svg width="44" height="44" viewBox="439 365 40 40" fill="none">
      <path
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const data = p.data as { id?: number; telegram_id?: string } | null
      if (data && (data.id != null || data.telegram_id)) {
        setProfile(p.data)
        const cur = useStore.getState().user
        setUser({ ...(cur || {}), ...p.data, photo_url: (p.data as { photo_url?: string | null }).photo_url ?? cur?.photo_url ?? null } as Profile)
      }
      setBonuses(b.data)
  ...
  const handleWithdraw = async () => {
    ...
    if (!effectiveFio) return showToast('Введите ФИО (полностью) для получения выплаты', 'error')
    ...
    try {
      ...
      const data = updated.data as { id?: number; telegram_id?: string } | null
      if (data && (data.id != null || data.telegram_id)) {
        setProfile(updated.data)
        const cur = useStore.getState().user
        setUser(cur ? { ...cur, ...updated.data, photo_url: (updated.data as { photo_url?: string | null }).photo_url ?? cur.photo_url ?? null } : updated.data)
      }
            ...
            {showDateFilter && (
              <div style={{
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: C.label, marginBottom: 3 }}>С даты</div>
                    <input
                      type="date"
                      value={txDateFrom}
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: C.label, marginBottom: 3 }}>По дату</div>
                    <input
                      type="date"
                      value={txDateTo}
              ...
              <>
                ...
                {txPage < txPages && (
                  <div style={{ textAlign: 'center', padding: '12px 0' }}>
                    <button onClick={() => loadTransactions(txPage + 1, txFilter, true, txDateFrom, txDateTo)} disabled={txLoadingMore} style={{
      ...
      {showWithdrawForm && (
        <div style={{
          ...
          <div style={{
            ...
            {/* ФИО — обязательно при первом выводе; после сохранения только для отображения (редактирует админ) */}
            <div style={{ marginBottom: 16 }}>
              ...
              {user?.full_name ? (
                ...
                <input
                  ...
                  value={wFio}
                  onChange={(e) => setWFio(e.target.value)}
                  style={{
              ...
              {user?.full_name && (
                <p style={{ fontSize: 11, color: C.sub, marginTop: 4 }}>
                  Изменить ФИО можно только через техподдержку
                </p>
              )}
            </div>
            {/* Method select */}
            ...
            <input
              type="text"
              inputMode={wMethod === 'card' ? 'numeric' : 'tel'}
