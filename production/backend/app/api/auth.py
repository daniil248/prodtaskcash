...
# IPs that should never be checked (local, docker networks, Telegram servers)
_SKIP_IP_PREFIXES = ("127.", "10.", "172.", "192.168.", "::1", "")


async def _check_ip_risk(ip: str | None) -> bool:
    """Returns True if IP appears to be a proxy, datacenter, or TOR node."""
    if not ip:
        return False
    for prefix in _SKIP_IP_PREFIXES:
    ...
    try:
        async with httpx.AsyncClient(timeout=2.0) as client:
            r = await client.get(
                ...
                params={"fields": "proxy,hosting,tor,status"},
            )
            if r.status_code == 200:
                d = r.json()
                if d.get("status") == "success":
                    return bool(d.get("proxy") or d.get("hosting") or d.get("tor"))
    except Exception:
        pass  # Non-blocking: if the check fails, don't penalise the user
    return False
...
async def telegram_auth(
    ...
    if not user:
        ...
        # VPN/прокси не штрафуем — у многих пользователей включён VPN. Только логируем для аналитики.
        ip_risky = await _check_ip_risk(client_ip)
        if ip_risky:
            logger.info(
                "Registration from proxy/VPN/datacenter (no penalty): telegram_id=%s ip=%s",
                telegram_id, client_ip,
            )

        # Дубль fingerprint → мультиаккаунт, снижаем стартовый trust
        if fp:
            ...
            if is_dup:
                ...
                trust_reason = "registration_dup_fingerprint"

        logger.info(
            "trust_score initial telegram_id=%s score=%s reason=%s",
            telegram_id, initial_trust, trust_reason,
        )
        user = User(
        ...
        # Смена устройства (телефон ↔ ПК, два устройства) — нормально, не штрафуем.
        # Раньше при совпадении fingerprint с другим аккаунтом снижали trust — это приводило к банам
        # при входе с двух устройств. Теперь только обновляем последнее устройство.
        if fp is not None:
