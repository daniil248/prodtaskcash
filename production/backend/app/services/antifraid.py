...
async def update_trust_score(
    db: AsyncSession, user: User, delta: int, reason: str | None = None
) -> None:
    """Изменяет trust_score, при падении ниже порога — автобан. Логирует все изменения."""
    old_score = user.trust_score
    new_score = max(0, min(100, old_score + delta))
    user.trust_score = new_score
    logger.info(
        "trust_score change telegram_id=%s old=%s delta=%s new=%s reason=%s",
        user.telegram_id,
        old_score,
        delta,
        new_score,
