...
export default function UsersPage() {
  ...
  const saveFio = async () => {
    ...
    try {
      ...
      setActionUser(data)
      if (detail) setDetail({ ...detail, user: data })
      ...
      {modal === 'detail' && actionUser && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" style={{ maxWidth: 560, maxHeight: '80vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
            ...
            {/* Редактирование ФИО (только админ; пользователь меняет через техподдержку) */}
            {actionUser && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 6 }}>ФИО для вывода</div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <input
                    value={editFio}
                    onChange={(e) => setEditFio(e.target.value)}
                    placeholder="Фамилия Имя Отчество"
                    style={{ flex: 1, padding: '10px 12px', borderRadius: 8, border: '1px solid var(--border)', fontSize: 14 }}
                  />
                  <button className="btn btn-primary btn-sm" onClick={saveFio} disabled={fioSaving}>
                    {fioSaving ? '...' : 'Сохранить'}
                  </button>
                </div>
              </div>
